...
const inputCls = (err) =>
  `w-full px-3 py-2.5 md:py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
...
export default function GrupoForm({ grupo, onSaved, onClose }) {
  ...
  const [form, setForm] = useState(() => isEdit && grupo ? {
    nombre:       grupo.nombre       ?? '',
    siglas:       grupo.siglas       ?? '',
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end md:justify-center md:items-center md:p-4">
      ...
      <div className="relative bg-white flex flex-col z-10 w-full rounded-t-2xl max-h-[90dvh] md:rounded-2xl md:w-[90vw] md:max-w-lg md:shadow-2xl">
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-4 bg-slate-800 rounded-t-2xl shrink-0">
          ...
          <h2 className="text-white font-bold text-base">
            {isEdit ? 'Editar Grupo / Familia' : 'Nuevo Grupo / Familia'}
          </h2>
          ...
          <form id="grupo-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              <Field label="Nombre" required error={errors.nombre}>
                <input type="text" name="nombre" value={form.nombre} onChange={handleChange}
                  placeholder="Ej: ILUMINACION" className={inputCls(errors.nombre)} />
              </Field>

              <Field label="Siglas (máx. 5)" required error={errors.siglas}>
                <input type="text" name="siglas" value={form.siglas} onChange={handleChange}
                  placeholder="Ej: ILM" maxLength={5}
                  className={`${inputCls(errors.siglas)} font-mono uppercase tracking-widest`} />
              </Field>
            </div>

            <Field label="Relación" required error={errors.relacion}>
            ...
            <Field label="Observación" error={errors.observacion}>
              <textarea name="observacion" value={form.observacion} onChange={handleChange} rows={3}
                ...
                className={`${inputCls(errors.observacion)} resize-none`} />
            </Field>
          </form>
        </div>
        ...
        <div className="shrink-0 flex gap-3 px-4 md:px-6 py-4 border-t border-slate-100 bg-white rounded-b-2xl">
          <button type="button" onClick={onClose}
            className="flex-1 py-2.5 md:py-2 text-sm font-medium bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg transition-colors">
            Cancelar
          </button>
          <button type="submit" form="grupo-form" disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 md:py-2 text-sm font-medium bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg transition-colors">
