      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={() => onDelete(g)} aria-label="Eliminar"
            ...
            className="w-11 h-11 flex items-center justify-center rounded-lg text-rose-500 hover:bg-rose-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
...
export default function GrupoList() {
  ...
  async function cargar(q = buscar) {
    ...
    setDeleteMsg(null)
    try {
      setGrupos(await getGrupos(q ? { buscar: q } : {}))
    } catch (e) {
      setError(e.message)
    } finally {
      setLoading(false)
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button onClick={() => { setEditItem(null); setShowForm(true) }}
          className="hidden md:flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm shrink-0">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Nombre o siglas..." value={buscar}
              ...
              className="w-full pl-9 pr-3 py-2.5 md:py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="grid grid-cols-2 gap-2 md:flex md:gap-2">
            <button onClick={() => cargar()} disabled={loading}
              className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white px-4 py-2.5 md:py-2 rounded-lg text-sm font-medium transition-colors">
              {loading ? <><Loader2 size={14} className="animate-spin" />Buscando...</> : <><Search size={14} />Buscar</>}
            </button>
            <button onClick={() => { setEditItem(null); setShowForm(true) }}
              className="md:hidden flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-800 text-white px-4 py-2.5 rounded-lg text-sm font-medium">
              <Plus size={14} />Nuevo
            </button>
          </div>
        </div>
      ...
      {showForm && (
        <GrupoForm grupo={editItem} onSaved={handleSaved}
